...
// links to the round on Lichess until it goes live and joins our strip.
...
export function UpcomingCard({ round, now, className = "" }: { round: UpcomingRound; now: number; className?: string }) {
  ...
  const body = (
    <>
      <div className="flex items-center justify-between gap-2 border-b border-border px-4 py-3 text-sm font-bold">
        ...
        {round.url && <ArrowUpRight className="size-4 shrink-0" aria-hidden />}
      </div>
  ...
  const card = `flex flex-col overflow-hidden rounded-lg border border-border bg-card ${className}`
  return round.url ? (
    <a
      href={round.url}
      target="_blank"
      rel="noreferrer"
      aria-label={`${title}, ${round.round}, on Lichess`}
      className={`${card} transition-colors hover:border-line-strong focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none`}
    >
      {body}
    </a>
  ) : (
